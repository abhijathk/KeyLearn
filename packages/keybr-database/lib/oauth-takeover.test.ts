...
test("EXPLOIT: a tenant-controlled email cannot take over an account", async () => {
  ...
  const before = await User.findByEmail("victim@keybr.com");
  ...
  equal(after!.email, "victim@keybr.com");
  equal(after!.externalIds!.length, 0);
  equal(
    (await User.loginWithPassword("victim@keybr.com", "correct horse battery"))
      ?.id,
    victimId,
  );
});
