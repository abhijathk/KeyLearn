...
export class Room {
  ...
  readonly #id: number;
  /**
   * How many people share one race.
   *
   * Ten is the ceiling because ten rails is about as much as anyone can
   * actually watch — past that the page stops being a race and becomes a
   * spreadsheet. Nobody ever queues for a place: `Game.#joinPlayer` opens a
   * fresh room the moment none has space, so a busy evening produces many small
   * rooms rather than one crowded one.
   */
  /** The configured size, for a room that does not exist yet. */
  static defaultSize(): number {
    return Math.max(2, Number(process.env.MULTIPLAYER_ROOM_SIZE || 10));
  }

  readonly #roomSize: number = Math.max(
    2,
    Number(process.env.MULTIPLAYER_ROOM_SIZE || 10),
  );
  ...
  /** How many this room seats, for the door to show before anyone connects. */
  get capacity(): number {
    return this.#roomSize;
  }

  /** Where this room is up to, so the door can say what you are walking into. */
  get state(): GameState {
    return this.#gameState;
  }

  /** Seconds until the next race, or 0 when one is not counting down. */
