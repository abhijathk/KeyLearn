import { type KeySample } from "@keylearn/result";
 ...
 * Deliberately gentle.
 *
 * The first cut of these halved a key's half-life on every miss down to a floor
 * of six hours, which meant the keys a learner was still working on — the ones
 * with misses in their history — were the ones that decayed hardest. Measured:
 * a struggling key came back the next day at six percent of its confidence.
 * Forgetting is real, but it is not that, and a model that overstates it turns
 * a night's sleep into a punishment.
 *
 * So: a floor of three days rather than six hours, and a miss now costs about a
 * seventh of the half-life instead of half of it. A neglected key still fades —
 * over a fortnight, which is roughly how long it takes.
 */
export const defaultDecayParams: DecayParams = {
  baseHalfLife: 7 * MS_PER_DAY,
  minHalfLife: 3 * MS_PER_DAY,
  maxHalfLife: 90 * MS_PER_DAY,
  growth: 0.15,
  shrink: 0.85,
};
