...
const SELF = "https://www.keylearn.org";

test("our own pages connect", () => {
  isTrue(allowWebSocketOrigin(SELF, SELF));
  isTrue(allowWebSocketOrigin("HTTPS://WWW.KEYLEARN.ORG", SELF), "case");
});
...
test("another site cannot open a socket from a visitor's browser", () => {
  ...
  isFalse(allowWebSocketOrigin("http://www.keylearn.org", SELF), "scheme");
  isFalse(allowWebSocketOrigin("https://keylearn.org", SELF), "host");
  isFalse(allowWebSocketOrigin("https://www.keylearn.org.evil.test", SELF));
});
