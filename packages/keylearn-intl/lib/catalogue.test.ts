...
const catalogue: Record<string, string> = JSON.parse(
  readFileSync(`${root}/packages/keylearn-intl/translations/en.json`, "utf8"),
);
...
test("every message the code asks for is in the catalogue", () => {
  ...
  equal(
    missing.join(", "),
    "",
    "these ids render as their hash — add them to packages/keylearn-intl/translations/en.json",
  );
});
