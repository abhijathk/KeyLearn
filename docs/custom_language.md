...
1. Add new language definition in `packages/keylearn-keyboard/lib/language.ts` and
...
2. Add your language definition to `ALL` enum in
   `packages/keylearn-keyboard/lib/language.ts`. Enum is sorted alphabetically.

## Phonetic model

1. Add dictionary of words in
   `packages/keylearn-generators/dictionaries/dictionary-<language-id>.csv.gz`.
...
3. Run command
   ```shell
   npm --workspace packages/keylearn-generators run generate-languages
   ```
   ...
   ```
   Files `packages/keylearn-phonetic-model/assets/model-<language-id>.data` and
   `packages/keylearn-content-words/lib/data/words-<language-id>.json` should be
